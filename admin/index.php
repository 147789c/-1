...
$(document).ready(function(){
	$('#title').html('正在加载数据中...');
            ...
            var html = '';
            if (data['status'] === 0) {
                html += `<li><a href="javascript:void(0);">${res['msg']}<span class="pull-right text-warning">警告</span></a></li>`;
            } else if (data['status'] === 2) {
                html += `<li><a href="./update.php">${res['msg']}<span class="pull-right text-danger">信息</span><div><span class="text-danger">最新版本</span><span style="margin-left:5px;" class="text-info">${data['data']['title']}</span></div></a></li>${data['data']['desc']}`;
            } else if (data['status'] === 1) {
                html += `<li><a href="javascript:void(0);">${res['msg']}<span class="pull-right text-success">提示</span></a></li>`;
            }
