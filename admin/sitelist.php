...
<input type="text" class="form-control" name="kw" placeholder="请输入代理用户名或域名"><br/>
...
<div class="block-title"><h3 class="panel-title">代理域名批量修改</h3></div>';
	echo '<div class="">';
...
<input type="submit" class="btn btn-primary btn-block" value="确定修改"></form>';
	echo '<br/><a href="./sitelist.php">>>返回代理列表</a>';
	echo '</div></div>';
}
else 
{
		...
		else 
		{
			...
			else 
			{
				...
				else 
				{
					...
					if ($DB->query($sql)) 
					{
						showmsg('批量修改域名成功！<br/><br/><a href="./sitelist.php">>>返回代理列表</a>',1);
					}
					else 
					{
	...
	else 
	{
		...
		{
			echo '<div class="block">
<div class="block-title"><h3 class="panel-title">添加一个代理</h3></div>';
			echo '<div class="">';
...
<div class="form-group">
<label>绑定域名:</label><br>
<input type="text" class="form-control" name="domain" value="" placeholder="代理要用的域名" required>
</div>
<!--div class="form-group">
<label>额外域名:</label><br>
...
<input type="submit" class="btn btn-primary btn-block" value="确定添加"></form>';
			...
			else 
			{
				if ($my=='edit') 
				{
					...
					$row=$DB->get_row('select * from shua_site where zid=\''.$zid.'\' limit 1');
					echo '<div class="block">
<div class="block-title"><h3 class="panel-title">修改代理信息</h3></div>';
					echo '<div class="">';
					echo '<form action="./sitelist.php?my=edit_submit&zid='.$zid.'" method="POST">
<div class="form-group">
<label>代理类型:</label><br>
<select class="form-control" name="power" default="'.$row['power']."\"><option value=\"1\">普及版</option><option value=\"2\">专业版</option></select></div>".($row['power']==1?'<div class="form-group">
<label>上级站点ID:</label><br>
<input type="text" class="form-control" name="upzid" value="'.$row['upzid']."\" disabled></div>":NULL).'<div class="form-group">
<label>绑定域名:</label><br>
				...
				else 
				{
					if ($my=='add_submit') 
					{
						...
						else 
						{
							...
							if ($DB->query($sql)) 
							{
								showmsg('添加代理成功！<br/><br/><a href="./sitelist.php">>>返回代理列表</a>',1);
							}
							else 
							{
					...
					else 
					{
						if ($my=='add2_submit') 
						{
							...
							else 
							{
								...
								{
									showmsg('添加代理成功！<br/><br/><a href="./userlist.php">>>返回用户列表</a><br/><a href="./sitelist.php">>>返回代理列表</a>',1);
								}
								else 
								{
									showmsg('添加代理失败！'.$DB->error(),4);
						...
						else 
						{
							if ($my=='edit_submit') 
							{
								...
								else 
								{
									...
									{
										showmsg('修改代理成功！<br/><br/><a href="./sitelist.php">>>返回代理列表</a>',1);
									}
									else 
									{
							...
							else 
							{
								if ($my=='delete') 
								{
									...
									if ($DB->query($sql)) 
									{
										showmsg('删除成功！<br/><br/><a href="./sitelist.php">>>返回代理列表</a>',1);
									}
									else 
									{
...
<a href="./sitelist.php?my=add" class="btn btn-primary">添加代理</a>&nbsp;<a href="#" data-toggle="modal" data-target="#search" id="search" class="btn btn-success">搜索</a>&nbsp;<a href="#" data-toggle="modal" data-target="#search2" id="search2" class="btn btn-warning">分类查看</a>&nbsp;<a href="javascript:listTable(\'start\')" class="btn btn-default" title="刷新代理列表"><i class="fa fa-refresh"></i></a>
